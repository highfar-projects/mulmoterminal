...
| Cursor CLI | shipped, GUI MCP (#2066) and the token badge (row 15) included. Still open: the transcript `.jsonl` for row 12, and `ctx %`, which needs cursor to name a model on an Auto session |
