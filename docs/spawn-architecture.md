...
| 2 | `cwd` | `CLAUDE_CWD` (launcher `--cwd` / env, default `~/mulmoclaude`) | Directory claude runs in. **Also scopes** which `.claude/skills` and (if enabled) `.mcp.json` are picked up, and which `<claude config home>/projects/<encoded cwd>` session list the sidebar shows (`CLAUDE_CONFIG_DIR`, default `~/.claude`) | Change → a different project + session list; missing dir is `mkdir -p`'d |
