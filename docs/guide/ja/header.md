...
| 1 段目 右 | 拡大・スリープ・閉じるなど**セルの操作** | 変えられません（アプリの構造） |
...
| `pr` | 現在のブランチの PR をブラウザで（**PR が無いときはボタン自体が出ません**） |
...
> **1 つのボタンには 1 つだけ書いてください。** 複数書くと、上の表の順で**最初の 1 つだけ**が
> 効き、残りは黙って無視されます。
...
`.claude/skills` が先、次に `~/.claude/skills`）。選ぶと**今のセッション**でそれを実行します
