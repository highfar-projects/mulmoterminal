...
- **ペインではそのセルのリポジトリが先頭に来ます**。open なものが 1 件も無い場合も、空のセクション
  が先頭に出ます（「無い」も答えなので）。セルのディレクトリが git リポジトリでない・`origin` が
  無い・このアプリが読めない forge にある・作業ディレクトリとして登録していない clone、のいずれか
  なら従来の設定順のままです。
