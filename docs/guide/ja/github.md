...
**エージェントを選ぶ。** 一覧の上の **Issue の開始に使う** で、どのエージェントで始めるかを選べます。
初期値は Claude（`defaultAgent` を設定していればそれ）で、選んだものはこのブラウザが覚えます。
Claude と Codex は、[アカウント](config.html#accounts)を設定していれば隣の **アカウント** でログインも選べます。
この環境にインストールされていないエージェントは、一覧に出ますが選べません。**Claude 以外は起動するとすぐ
Issue の本文を実行します**（下書きを置いておく入力欄がないため）。さらに Codex 以外はツールの使用も自動で
承認するので、それらを選んでいる間は警告を出し続けます。セッションが既にある Issue は、この選択に関係なく、
そのセッションを始めたときのエージェントとログインで再開します。
