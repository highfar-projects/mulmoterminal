...
**ワークスペースで起動したセッションは、GUI ツールを全部持ちます** — Canvas に描かせる・コレクションを触らせるといったツールが、何も登録せずに通る状態です。
Claude でも Codex でも、セルからでもランチャの **WORKSPACE** チップからでも同じです（Antigravity だけは例外で、どこでもディレクトリに登録されたぶんだけ）。
