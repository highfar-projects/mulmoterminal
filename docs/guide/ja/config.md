...
- 組み込み `dir` / `git` / `work` / `diff` / `ctx` / `usage` / `status` / `tools` / [`env`](#worktree-env) … 並べた順に表示、書かなければ非表示。
...
プロジェクト本体の checkout は宣言した `base` をそのまま使い、worktree はその上の番号を取ります。
...
- 宣言しなければ何も変わりません。`worktreeEnv` が無ければ予約もファイル書き込みも変数設定も
  一切起きません。
