...
- メニューが出るのは**エージェントのセル**だけです。対象は MulmoTerminal がデッキを配信するディレクトリ —— 起動したワークスペースに加えて、ランチャーに保存したディレクトリすべて。これらは**起動時に一度だけ**読むので、初めて開くリポジトリは再起動が要ります。それ以外のデッキは、ファイルツリーの右クリック（**Open in the Canvas**）から開けます。
