...
新しいアカウントで最初に起動するセルは、**空の**設定ディレクトリで始まります。そのため、Claude Code がそこでログインを求めます（`/login`）。ログインはそれ以降、そのディレクトリに保存されます。設定・自分で登録した MCP サーバー・プロジェクトごとの信頼の回答はいつものログインのものなので、これらも空から始まります。一方、MulmoTerminal が用意するものはアカウントでも使えます。同梱の `mulmoterminal-*` スキルはそのアカウントのディレクトリにも入り、アカウントのセルにも、いつものログインのセルと同じように、ディレクトリの GUI ツール（起動画面のスイッチ）が渡されます。
