...
プロジェクトごとの既定は、そのプロジェクトの `.mulmoterminal.json` に `"account": "work"` と書きます（この fork 独自）。起動画面の ACCOUNT が **Default login** のままなら、新しいセッションはこのアカウントで起動します。別のエージェント用のアカウントの id は、そのエージェントでは無視されます。worktree にも引き継がれます。
