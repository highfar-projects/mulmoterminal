...
| `files-find` | 今のターミナルの横の Files ペインで、**ファイル名から探して開く**。名前やパスの一部を打って候補から選ぶと、ツリーもそのファイルの位置まで開いた状態で開きます。git リポジトリなら候補は git から取るので `.gitignore` が効き、そうでないディレクトリは走査するだけで何も除外されません。ペインが閉じていれば先に開きます | 必要 |
