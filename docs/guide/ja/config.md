...
- **タイル表示中に来た質問は失われません。** セッションはその質問で止まったままなので、**そのセルを
  拡大した時点で**ペインが開きます。ペインを開くのはこの 2 つだけで、専用のボタンはありません。
  Esc で閉じたことはそのダイアログについて記憶されるので、セルに戻っても開き直しません。
  次の質問は通常どおり開きます
...
OFF のとき、サーバは何も publish しません。つまり質問がそもそもブラウザに届きません ——
ペインが隠れているのではなく、出すものが無い状態です。
