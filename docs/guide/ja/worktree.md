...
しかも、その 2 つの `yarn dev` がポート 3000 を取り合うこともありません。
[`worktreeEnv`](config.html#worktree-env) を宣言したプロジェクトには **worktree ごとに**
ポートと DB 名が配られ、その worktree のターミナルに環境変数として渡り、セルのヘッダに
クリックできる `:3010` として出ます。
