...
|---|---|---|
| **ワークスペースと同じ** | **GUI ツールを全部持ちます。登録は要りません。**設定の [MCP servers](config.html#settings-modal)（`userMcpServers`）もここに合流します | 特例はありません。**そのディレクトリに登録されているツールグループだけ**です |
| **プロジェクトのディレクトリ** | **そのディレクトリに登録されているツールグループだけ**です。GUI ツールが要るなら MCP トグルで登録します | 同じ |

どちらのディレクトリでも、**あなた自身の MCP サーバ**（`.mcp.json`・`claude mcp add`・claude.ai のコネクタ）は普通に読まれます。4.4.0 より前はワークスペースのセルだけがそれを読めていませんでした（→ [4.4.0 セットアップガイド](v4.4.0.html)）。
...
**エージェントは問いません** — Claude でも Codex でも、セルからでもランチャのチップからでも同じツールに届きます（→ [4.3.0 セットアップガイド](v4.3.0.html)）。
...
![ランチャのチップ列 — 先頭がワークスペース](../images/v4.3.0-workspace-chip.png)
