...
> **5.1.0 が出ました。** キーマップに **`focus-next` / `focus-prev`** が加わり、**タイル表示**の
> グリッドをキーボードカーソルで移動できるようになりました。`zoom-next` / `zoom-prev` の拡大して
> いないとき版で、マウス無しで隣のターミナルへ行ける初めての手段です。opt-in なのでキーの割り当て
> が要ります。あわせて、**cockpit roster の `prompt` / `reply` 行が codex と cursor のセルでも
> 埋まる**ようになりました（これまで空でした）。[セットアップガイド](v5.1.0.html)
