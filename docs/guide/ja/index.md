...
> **5.3.0 が出ました。** Files ペインが、名前で探すだけでなく**ファイルの中身を検索**できるように
> なりました。開いている **Markdown プレビューがディスク上の変更に追随**するので、手でやっていた
> リロードが消えます。そしてペインが**開いていたファイル・表示モード・読んでいた行に戻ってくる**ように
> なり、読み終える前に「空のディレクトリ」と言う代わりに `Loading…` と出ます。
> [セットアップガイド](v5.3.0.html)
