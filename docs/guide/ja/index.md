...
> **4.24.0 が出ました。ShapeScript のツールが 1 つ名前を変えています。** `publishShapeScript` は
> 無くなり、ギャラリーは **`manageShapeScript`**（`action` を持つ 1 本のツール）になりました。
> 公開だけでなく**更新・削除・一覧**もできます。設定の中に古い名前を書く場所はありませんが、
> ご自分で保存したプロンプトには書いてあるかもしれません。結果を **GLB / STL** で持ち出すことも
> できるようになりました。
> [何を直すか / 入ったかの確かめ方](v4.24.0.html)。
