...
> **5.8.0 が出ました。** セルを **2 つ目の Claude Code / Codex の契約**で動かせるようになりました
> （ベータ）。そのアカウントの 5h / 7d の使用枠もツールバーに出ます。このマシンで起動できない
> エージェントは、ピッカーでインストール手順へのリンクつきで薄く表示されます。
> [セットアップガイド](v5.8.0.html)
