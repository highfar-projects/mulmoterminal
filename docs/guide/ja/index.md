...
> **[4.7.1 で変わったこと](v4.7.1.html)** — **設定画面が「いま動いているビルド」を教えてくれる**ようになった。npm 版はバージョン、git チェックアウトはバージョンと短い HEAD **コミット**。新しいものがあれば更新通知もその行に出る。設定は不要。**Agent Picker** は各エージェントに固有のマークが付き、どの行がどのツールを起動するのか見て分かるようになった。そして**長すぎる（4,096 バイト超、全プラットフォーム）／改行を含む（Windows）シードプロンプト**でセッションが死ななくなった（ファイル経由で渡す）（2026-08-07 時点）
>
