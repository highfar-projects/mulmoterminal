...
> **4.26.0 が出ました。「already running」と出たまま止まらなくなりました。** スクリプトやラッパーから
> 誰もキーボードの前にいない状態で起動すると、あの通知を出したきりポートを bind しないことが
> ありました。強制終了で残った古いエントリは、信じるのではなく削除するようになり — 手で消す必要は
> もうありません — 誰も答えない起動時の質問は「答えられる人がいなかった」として扱われます。
> あのプロンプトでの **Ctrl+C** は従来どおり「断る」です。
> [何が起きていたか、直っているかの見分け方](v4.26.0.html)。
