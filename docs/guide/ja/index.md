...
> **4.23.0 が出ました。cursor のセルが、他のセルに追いつきました。** ランチャの
> **Canvas / Workspace data / External accounts** のスイッチが **Cursor** にも届くようになり、
> ヘッダーに**トークンバッジ**が出て、終わったターンが**何と答えたか**を持つようになりました
> —— 完了通知がそれを引用し、handoff が別のセルへ渡せます。設定が要るのはスイッチだけです。
> [何を ON にするか / 効いたかの確かめ方](v4.23.0.html)。
