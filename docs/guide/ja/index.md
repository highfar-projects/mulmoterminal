...
> **5.2.0 が出ました。** **cockpit roster をドラッグで並べ替え**できるようになり（手動ソート時）、
> **`summary` 行が全エージェントで埋まる**ようになりました — これまで Claude だけだったところ、
> codex / cursor / Antigravity / grok / copilot / muse がそれぞれ自分のストアから答えます。あわせて、
> `Cmd+Shift+<英字>` の keymap は**英字を小文字で書かないと macOS で発火しません**。警告が出るように
> なったので、お使いの設定を確認してください。[セットアップガイド](v5.2.0.html)
