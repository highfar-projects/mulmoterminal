...
> **4.11.0 が出ました。** 動いているサーバをどのターミナルからでも、あるいはブラウザから止められる
> ようになり、ローカルのセッションを静かに壊していたポートの不具合 3 件が直りました —— `PORT` が
> 効くようになり、埋まっているポートでの二重起動は始める前に止まります —
> [何が変わって、何をすればいいか](v4.11.0.html)。
