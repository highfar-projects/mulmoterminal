...
> **5.7.0 が出ました。** ホイールで遡って tmux の copy-mode に入ったセルが、打った文字を黙って
> 飲み込むのではなく、**そのことを表示し、「入力に戻る」ボタンを出す**ようになりました。スマホの
> ターミナル一覧も、実行中のセッションを ID ではなく会話の内容で名付けます。
> [セットアップガイド](v5.7.0.html)
