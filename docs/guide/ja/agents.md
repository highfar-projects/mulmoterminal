...
- **Cursor** は `.cursor/mcp.json`（または `~/.cursor/mcp.json`）を読みます。**このファイルを書く処理は
  まだ入っていません**（[#2066](https://github.com/receptron/mulmoterminal/issues/2066)）。つまり今の
  cursor セルは、あなた自身が設定した MCP にだけ到達します。

なので Antigravity か Grok を選んでいるときは、ワークスペースでも4つのトグルが出たままになります。
それが正確な答えで、ディレクトリのファイルがこの2つにとって GUI ツールを得る唯一の道だからです。
**トグルは Cursor にはまだ届きません** — 書き換えるのは Antigravity と Grok のファイルであって
`.cursor/mcp.json` ではないので、cursor のセルは #2066 が入るまでトグルを無視します。
