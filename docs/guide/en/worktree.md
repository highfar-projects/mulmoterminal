...
And two of those `yarn dev`s no longer fight over port 3000. A project that declares
[`worktreeEnv`](config.html#worktree-env) is given **a port and a database name per worktree**,
exported into that worktree's terminals and shown on the cell header as a clickable `:3010`.
