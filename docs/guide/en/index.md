...
> **[What's new in 4.7.6](v4.7.6.html)** — **groundwork for collections outside the workspace**: nothing to configure and nothing new on screen. The collection engine stops assuming one shared workspace and **resolves a root per request**, so a request may name a project and be served that project's directory — the part that has to come first before a collection can live in any project directory you work in. The UI for picking a project is not built yet, so nothing in the browser sends one. `@mulmoclaude/core` moved to **3.0.1**, which also closes a way two copies of the GUI panel's runtime could have ended up loaded at once (as of 2026-08-09)
>
