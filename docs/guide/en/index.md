...
> **[What's new in 4.7.1](v4.7.1.html)** — **Settings now says which build you are running**: the version for an npm install, and the version plus the short HEAD **commit** for a git checkout, with the update notice repeated there when something newer exists. Nothing to configure. The **Agent Picker** gives every agent its own mark, so a row says which tool it starts. And a **seed prompt** that is too long (over 4,096 bytes, any platform) or carries newlines (Windows) no longer kills the session — it goes to a file instead (as of 2026-08-07)
>
