...
Claude, Codex or Copilot, it is the same, and the **WORKSPACE** chip at the head of the WORKING DIRECTORY row is the quick way to get there (Antigravity, Grok and Muse are the exceptions: wherever one of them runs, it gets what its directory registered, and Cursor gets no GUI tools from MulmoTerminal at all yet → [Antigravity, Grok and Muse register everywhere](basics.html#antigravity-gui-tools)). A **launch command** is not this — it runs your command line verbatim and carries no GUI tools, `claude` included.
