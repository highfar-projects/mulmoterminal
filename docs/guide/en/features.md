...
| Mulmo menu (**Show a deck from this directory**) | Lists this project's mulmoScript **decks** and shows the picked one in the **Canvas** beside the cell. A viewer: nothing is typed into the session and the agent is not asked, so it costs no tokens. Two named sources and no search — the workspace's `artifacts/stories`, plus the paths you list in `decks` in `.mulmoterminal.json`. Works in the workspace and in every directory your launcher has saved (read once at startup); any other deck is still reachable from the file tree's **Open in the Canvas** |
