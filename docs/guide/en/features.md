...
| Conversation pane | The session's **conversation as turns**, beside the enlarged cell — read from the agent's own transcript, not the screen, so a Claude cell's alternate screen no longer means "what scrolled past is gone". **Scroll up to walk back to the first turn**, a page at a time. Prose wraps, fenced code stays monospace. A snapshot with a reload, not a live tail. Claude, Codex, Cursor, Copilot |
