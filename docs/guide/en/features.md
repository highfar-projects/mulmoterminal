...
| Mulmo menu (**Show a deck from this directory**) | Lists the mulmoScript **decks** kept under that directory and shows the picked one in the **Canvas** beside the cell. A viewer: nothing is typed into the session and the agent is not asked, so it costs no tokens. Decided by the marker inside the file, not the `.json` extension; bounded to 4 directories deep, 50 decks, 500 files opened and 2000 directories listed. Only under the workspace stories are served from — the same decks the file tree's **Open in the Canvas** reaches |
