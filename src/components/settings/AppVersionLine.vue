...
<template>
  <div v-if="display" class="mt-1 flex flex-col gap-1">
    <div class="flex flex-wrap items-center gap-1.5">
      ...
      <span class="text-[11px] font-semibold uppercase tracking-[0.04em] text-muted">Version</span>
      <span :class="CHIP" class="text-fg" data-testid="settings-app-version">{{ display.version }}</span>
      <span v-if="display.commit" :class="CHIP" class="text-muted" data-testid="settings-app-commit">commit {{ display.commit }}</span>
    </div>
