...
export const ja: Messages = {
  settings: {
    ...
    shortcuts: {
      intro:
        "読み取り専用で、割り当ての有無にかかわらず全部 {keymapKey} の下に並びます。割り当てられるのは 2 種類 —— MulmoTerminal の操作（拡大、待っているエージェントへ移動、コピー / ペースト）と、ターミナルへのキー列送信（macOS で Cmd+← を行頭へ、など）。割り当てたキーはターミナル内のプログラムに届かなくなるので、下のボタンから設定してください —— エージェントが、あなたのターミナルでいま動いているものが既に使っているキーと突き合わせてから書き込みます。{guide}にリファレンスがあります。",
      guide: "ガイド",
      list: "キーボードショートカット",
      notSet: "未設定",
