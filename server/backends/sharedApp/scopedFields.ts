...
export function scopedFieldProblems(app: AuthoredApp, schemas: readonly { cid: string; schema: CollectionSchema }[]): string[] {
  ...
  return [...assigneeFieldProblems(app, byCid), ...stampFieldProblems(app, byCid), ...windowFieldProblems(app, byCid)];
}

/** The field that says whose row it is. */
function assigneeFieldProblems(app: AuthoredApp, byCid: Map<string, CollectionSchema>): string[] {
  return Object.entries(app.collections ?? {}).flatMap(([cid, config]) => {
    const name = config.assigneeField;
    const schema = byCid.get(cid);
    if (name === undefined || schema === undefined) return [];
    const spec = fieldOf(schema, name);
    if (spec === undefined) {
      return [
        `collections.${cid}.assigneeField names '${name}', which '${cid}' does not declare. The rules compare that field with the signed-in address to decide ` +
          "whose row it is, so every write by an assignee is refused — and nobody else notices.",
      ];
    }
    if (ADDRESS_TYPES.has(spec.type)) return [];
    return [
      `collections.${cid}.assigneeField names '${name}', which is a '${spec.type}' field. It has to hold an email ADDRESS, because that is the only thing the rules ` +
        `can compare a member against. A ref stores the target's primary key, not an address — declare a plain string or email field beside it and let the ref stay ` +
        "the thing the UI renders.",
    ];
  });
}

/** The field pinned to the server clock. */
function stampFieldProblems(app: AuthoredApp, byCid: Map<string, CollectionSchema>): string[] {
  return Object.entries(app.public?.submit ?? {}).flatMap(([cid, submit]) => {
    ...
    if (spec === undefined) {
      return [
        `public.submit.${cid}.stampField names '${name}', which '${cid}' does not declare. The rules require the record to CARRY the server time in that field, ` +
          "so nothing can be submitted at all.",
      ];
    }
    if (spec.type === "datetime") return [];
    return [
      `public.submit.${cid}.stampField names '${name}', which is a '${spec.type}' field. What the rules write there is \`request.time\`, a timestamp — declare it as ` +
        "`datetime`, or the comparison is a type error and every submission is denied.",
    ];
  });
}

/** The bound that lives on another record. */
function windowFieldProblems(app: AuthoredApp, byCid: Map<string, CollectionSchema>): string[] {
  return Object.entries(app.public?.submit ?? {}).flatMap(([cid, submit]) => {
    ...
    if (refSpec === undefined) {
      problems.push(
        `public.submit.${cid}.window.fromField.ref names '${ref.ref}', which '${cid}' does not declare. That field is where the rules read WHICH record carries the ` +
          "opening time, so without it nothing opens.",
      );
    ...
    const target = byCid.get(ref.collection);
    ...
    if (opens === undefined) {
      problems.push(
        `public.submit.${cid}.window.fromField.field names '${ref.field}', which '${ref.collection}' does not declare. Each record there has to carry its own opening ` +
          "time, or the window never opens for any of them.",
      );
    } else if (opens.type !== "number") {
      problems.push(
        `public.submit.${cid}.window.fromField.field names '${ref.field}', which is a '${opens.type}' field in '${ref.collection}'. It must be a 'number' holding EPOCH ` +
          "MILLIS: the rules compare it with `request.time.toMillis()`, and comparing that with anything else is a type error that denies every submission. Whoever " +
          'schedules the record computes the value ("three days before, at 08:00" is business knowledge, and the rules have no usable date arithmetic).',
      );
    }
    return problems;
  });
