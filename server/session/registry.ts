...
// When a session was `/clear`ed, so a reader can tell this conversation's material from the one
// the user ended. `clearedTranscripts` answers WHETHER, which is all its own readers need; the
// prompts pane needs WHEN, because claude's prompt history is one file per user rather than one
// per conversation and a clear leaves no seam in it (#1749).
//
// In memory: the durable mark records the transcript's SIZE, not a time, so a restart forgets the
// boundary and a cleared session's older prompts can reappear until it is cleared again. Bounded
// and stated rather than fixed here — putting a time into that mark changes a persisted format
// whose expiry rule is load-bearing (cleared-transcripts.ts).
export const clearedAtMs = new Map<string, number>(); // our id -> epoch ms of the `/clear`
