...
server.listen(Number(PORT), BIND_HOST, () => {
  onListening({ server, loopbackServers, browserHostnames });
  // The devcontainer twin of the loopback reachability problem, and the reason it is a separate
  // call: a session in its own network namespace cannot reach ANY port on this machine, so the hook
