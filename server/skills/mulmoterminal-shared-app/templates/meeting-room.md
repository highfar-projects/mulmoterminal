...
>    行の中身ではありません — 各行は `id` / `startAt` / `opensAt` / `closesAt` / `state` を
>    揃えた**完全なレコード**にすること（必須フィールドが欠けた行は `putItems` が拒否します）
> 3. 休業日・祝日は作らない（一覧は設定ファイルから読む）
> 4. 生成はスクリプトにやらせ、出力は**ワークスペース配下の JSON ファイル**に書く
> 5. そのファイルの絶対パスを `manageCollection` `putItems` の **`itemsFile`** に渡す。
>    **`mode: "create"` を必ず付ける**。1 回 1000 行までなので、超えるならファイルを
>    分けて複数回呼ぶ
> 6. 作った件数、`rejected`（＝既にあった枠）の件数、新しい最遠日を報告する
