...
> 2. 今日から 90 日先までで、**まだ存在しない枠の id だけ**を作る。生成はスクリプトに
>    やらせ、出力は**ワークスペース配下の JSON ファイル**に書く
> 3. 休業日・祝日は作らない（一覧は設定ファイルから読む）
> 4. そのファイルの絶対パスを `manageCollection` `putItems` の **`itemsFile`** に渡す。
>    1 回 1000 行までなので、超えるならファイルを分けて複数回呼ぶ
> 5. 作った件数と、新しい最遠日を報告する
