...
export function cursorPosterSource(): string {
  return `// Generated by MulmoTerminal. Posts one cursor hook payload to a local MulmoTerminal.
// The URL lives HERE and not on the command line: cursor silently refuses to run a hook command
// whose command line contains a URL, and one refused entry voids the whole hooks file.
...
export function syncCursorHooksFile(port: string | number, home: string = cursorHome(), mtHome: string = mulmoterminalHome()): void {
  ...
  try {
    ...
    writeAtomically(script, cursorPosterSource());
    if (existing === next) {
 ...
 *
 * NOT tidiness. The file names a bare port and outlives the process that wrote it, so a server that
 * has exited leaves cursor posting every prompt and tool argument to whatever takes that port next.
 *
