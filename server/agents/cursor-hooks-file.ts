...
// IS ANYONE THERE, and is it US? A hook file outlives the server that wrote it — a crash skips the
// exit handler, and a user who edits this file makes it one MulmoTerminal may no longer rewrite or
// remove. A stranded command must therefore be HARMLESS rather than merely unlucky: without this
// it posts every prompt and tool argument to a bare local port, which by then may belong to
// something else entirely.
//
// Fails OPEN when the registry cannot be read at all (a machine where it was never writable keeps
// the status it has today) and CLOSED when it CAN be read and names no live instance on this port.
function servedByLiveMulmoTerminal() {
  let entries;
  try {
    entries = fs.readdirSync(INSTANCES);
  } catch {
    return true; // no registry to consult — behave as this hook always has
  }
...
process.stdin.on("end", () => {
  ...
  // A server that is down must cost the user's turn a moment, not a minute, and must print nothing
  // into their terminal.
