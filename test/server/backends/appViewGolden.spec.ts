...
//
// Why a document and not just the projection's own unit tests (which live in
// `sharedapp` with the code). This one is a picture of the WIRE: the whole
// document, key order and all, in a file a reviewer can read in a diff. A
// change to `{tier}/config` that nobody meant shows up here as a diff rather
// than as a passing test somewhere else.
//
// Regenerate with `UPDATE_GOLDEN=1 yarn vitest run test/server/backends/appViewGolden.spec.ts`.
//
// It is NOT copied into mulmoserver any more. That copy existed because
// mulmoserver could not run the projection; now both repositories import it
