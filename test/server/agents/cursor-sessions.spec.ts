...
describe("cursorSessionExists", () => {
  it("finds a chat under any project directory — the survivor guard has no cwd to check against", () => {
...
describe("cursorSessionExistsForCwd", () => {
  it("resumes a chat only in the directory that recorded it", () => {
    ...
    expect(cursorSessionExistsForCwd("chat-2", CWD, home)).toBe(false);
  });

  it("declines when the project directory records no path — omission, never another directory's chat", () => {
    chat(project("slug-unattributable-333", null), "chat-3", "hello");
    expect(cursorSessionExistsForCwd("chat-3", CWD, home)).toBe(false);
  });
});

describe("listCursorSessionsForCwd", () => {
  it("lists only this directory's chats, titled by the first thing the user said", () => {
    const dir = project("slug-a-111", CWD);
    chat(dir, "chat-1", "<user_query>\nFix the build\n</user_query>");
    chat(project("slug-b-222", OTHER), "chat-2", "Something else");
    const metas = listCursorSessionsForCwd(CWD, home);
    expect(metas.map((m) => m.id)).toEqual(["chat-1"]);
    expect(metas[0].title).toBe("Fix the build");
    expect(metas[0].mtimeMs).toBeGreaterThan(0);
  });

  it("is empty for a directory cursor has never seen", () => {
    project("slug-a-111", CWD);
    expect(listCursorSessionsForCwd(OTHER, home)).toEqual([]);
  });

  it("skips a chat directory that has no transcript yet rather than failing the listing", () => {
    const dir = project("slug-a-111", CWD);
