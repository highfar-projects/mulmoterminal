...
describe("filesRowActions — the Canvas entry", () => {
  const WORKSPACE = "/ws";
  const inProject = { cwd: "/proj", terminal: { cwd: "/proj" }, canvas: { roots: { workspace: WORKSPACE, rootId: null } } };
  ...
  it("offers it on a story in the workspace, and not on one inside a project", () => {
    const inWorkspace = { cwd: WORKSPACE, terminal: { cwd: WORKSPACE }, canvas: { roots: { workspace: WORKSPACE, rootId: null } } };
