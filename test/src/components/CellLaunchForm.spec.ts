  ...
  describe("agent-filtered resume list (#1417)", () => {
    ...
    function mockFetchPerAgent(agentSessions: Record<string, SessionRow[]>) {
      globalThis.fetch = vi.fn(async (url: string) => {
        ...
        // Each agent has its own API path — check specific paths FIRST before the generic /api/sessions
        // because "/api/sessions" is a substring of "/api/codex/sessions" etc.
        let agent = "claude"; // default
        if (u.includes("/api/codex/sessions")) agent = "codex";
        else if (u.includes("/api/antigravity/sessions")) agent = "antigravity";
        else if (u.includes("/api/grok/sessions")) agent = "grok";

        if (u.includes("/api/") && u.includes("/sessions")) {
