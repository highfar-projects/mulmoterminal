...
6. **レイアウトの無いページへの `scrollTo` はクランプされる**ので、位置は「当てた」ことで失われる。
   これが本 PR で一番高くついた部分で、実機で回して初めて分かった:
   - 復元はスクロールを起こし、スクロールは報告を起こす。**復元直後の短い静粛時間**が無いと、
     クランプされた値が親に送り返されて上書きされる。
   - この文書の画像は寸法を持たないので読み込み中にページが伸びる。
   - そしてペインは Edit に切り替えると iframe を `display:none` にする。
     レイアウトが無いので**そこに届いた位置は全部クランプされて消える**
     （エディタで閉じたディレクトリに戻ったとき / エディタの裏でファイルが書き換わったとき）。
   → **文書が自分の高さを ResizeObserver で見て、伸びたら当て直す。**
   測って捨てた案が二つ: `resize` は**発火しない**（`innerHeight` は変わらない。変わるのは
   `scrollHeight` の方）。親側で「Preview を表示したら送り直す」も**3 回に 1 回しか通らない**
   （`display` が戻ることはレイアウトが済んだことではない）。
   読者が自分でスクロールしたら当て直しは止める。
