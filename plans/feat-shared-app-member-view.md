...
- 代償は、**公開ページが名簿を引く**こと。`apps/{aid}` は `readerOf` しか読めないので、
  匿名の訪問者にとっては「読めない＝公開の顔」で確定する（失敗はしない）。ただし
  公開の顔のコードに「自分は名簿にいるか」の分岐が入る
...
| 渡すデータ | `views[].collections ∩ public.read` | `views[].collections`（読めるかはルールが決める。読めなければ問題として言う） |
...
| mulmoterminal | — | — | — | `member/staged` と `member/view` の書き出しと撤去、テンプレート |
