...
| URL | 何を出す |
|---|---|
| `/m/{slug}` | 開けるビューが 1 つならそこへ**置き換え遷移**。2 つ以上なら**選択画面**。0 なら「このアプリにあなた向けのページはありません」 |
...
- **知らない `id` と、開けない `id` は同じ答え**にする（選択画面に戻し、
  ...
  区別しないのと同じ理由
- **「開ける」の判定は `live:config` に載っているか**。射影は階層ごとなので、
  参加者の `roster/live:config` にスタッフ用の `id` は最初から載らない
...
- **どの階層を読むかは、読めるかどうかで決める。** `/staging/{aid}` は名簿の人なら
  誰でも来られるので、階層を順に当たる: **`member/staged:config` → 拒否されたら
  `roster/staged:config`**。**拒否はエラーではなく階層の判定そのもの**で、
  参加者に権限エラーの画面を出してはいけない（決定 3 の入口の判別と同じ形）
- 読めた `staged:config` に載っている `id` を `staged:{id}` から bridge で描く
  （`/m/{slug}` と同じ機構・同じ射影）
