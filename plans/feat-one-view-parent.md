...
## 8. 残っていたクライアント側の取りこぼし（2026-08-19）

3 本を入れた翌日、実アプリ（`../apps/todos`）を触った作者が「完了にする」が無反応だと報告した。
サーバは通す気でいたのに、**ペインが送る前に捨てていた** — `sharedAppPreviewIntent.ts` に

```ts
if (current.audience === "public") return null;
```

が残っていた。`previewIntent.ts` からは同じ拒否（`not-a-member-page`）を消し、理由まで書いてあった
のに、その手前の 1 行を見落としていた。**サーバ側の判定を書き換えて満足し、同じ判断を持つ
クライアントを探さなかった**のが原因で、これは今回の変更そのものが除こうとした形（同じ判断が 2 箇所に
ある）である。

同じ根から、判定ではないところにも 2 つ残っていた。どちらも**「audience が何を決めるか」を
更新し損ねたもの**である。

- **拒否の語彙が audience で選ばれていた。** `REFUSALS`（submit 用）と `INTENT_REFUSALS`（intent 用）の
  2 枚があり、`explainRefusal(reason, audience)` が公開ページなら前者・会員ページなら後者を引いていた。
  親が 1 つになった後は、**どちらのページでもどちらの ask も起きる**ので audience は何も言っていない。
  公開ページの `read-only` や `illegal-transition` が生のまま作者に出ていた。
  → **1 枚に統合し、`explainRefusal(reason)` は引数を 1 つにした。** 衝突するのは
  `unknown-collection` だけなので、**両方の宣言を名指す**文にした（推測しない）。
  ログの `refused` / `intent` エントリからも `audience` を落とした（もう誰も読まない）。
- **capabilities 欠落の警告が会員ページ限定だった。** 公開ページも `viewer` を持つように変えたのに、
  警告文は「PUBLIC の親で動いている」という**撤回済みの世界観**のままだった。
  → 全 audience で出す。文面も「この親で動く」ではなく「`viewer.can` から描く操作が全部消える」に直した。

**教訓**: 判定を片側から消したら、**同じ問いをしている場所を名前で探す**（`audience === "public"`,
`Exclude<PreviewAudience, "public">`）。型が `public` を除外している場所は、除外という判断が
コードとして残っている場所である。
