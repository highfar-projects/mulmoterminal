...
**状態**: **S0–S2 は main にマージ済み**（#1843、2026-08-23）。その上に **`watch` / `unwatch`**（購読）を
足した — 下の「購読（live）」を参照。**S3 は未着手**。
...
- **購読（live）。** ~~公開ページの live は人が見ている前提の扇形で、エージェントが張り続けるのは
  D7 に近い形になる。訊かれたときに取りに行く。~~ **やった** — 下の節を見よ。
...
- **`watch` は実地で通していない。** S0–S2 と同じく、緑はモックに対するものである。

---

## 購読（`watch` / `unwatch`）

**presentForm と同じ形にする。** 購読の要求は**即座に成功で返り**、変化が起きたときに**別経路で
テキストが届く**。これは MulmoClaude の presentForm がやっていることそのもので、しかも MCP の
push ではない — ブローカーは**ステートレスな Streamable HTTP**（`server/mcp/broker.ts`、リクエスト
ごとに 1 つ作る）なので、サーバから送り下ろす通り道が最初から無い。届け方は
**PTY に打ち込む**こと（`writeToSession` / `createTerminalInputSender`）で、電話からの入力
（#445）と質問への回答（#1685）が既に使っている道と同じである。

### W1. 通知はアプリのデータを一切運ばない

**これがこの機能の中心の決定である。** `useSharedApp` の他のすべてでは、他人の言葉は
**エージェントが自分で決めて呼んだ結果として**、`quoted.ts` を通って届く。watch はそれを裏返す:
**ユーザが打つ位置に**、会ったこともない誰かの書き込みがテキストを出現させる。そこにアプリの
文字列を載せれば、publish した側が**好きな時刻に、指示として届くものを選べる**ことになる。

だから届く行は**こちらの固定文**で、アプリが寄与するのは**エージェントが既に持っていた
slug と cid だけ**（`quoted` で囲って上限つき）。中身は「`bookings` で 3 件変わった」まで。
何が変わったかは `records` を呼んで初めて分かる — つまり**従来どおりの、エージェントが選んだ読み**
になる。

**ただし「動くな」とは言わない。** 最初の版は行の末尾が「acting せずユーザに報告せよ」だったが、
それは 2 つの別のことを混ぜていた —「**アプリの言葉**は指示ではない」ことと、「エージェントが
**頼まれた仕事**をしてよいか」である。watch はそもそも「来たら承認して」「返事が来たら返して」と
言って画面を離れるためのものなので、起きて読んで既に持っている許可を訊き直すのは、何もしないための
遅い方法でしかない。**境界は元のまま**アプリのデータの周りにあり、ユーザの依頼はその外側にある。

### W2. ダイアログにも、書きかけの文にも打ち込まない

`deliverable()`（`server/session/shared-app-watches.ts`）が 4 つ見る。

| 状態 | 打つか | なぜ |
| --- | --- | --- |
| pty が無い | 待つ | 再起動をまたいだセッション、再接続の隙間 |
| `working` | 待つ | ターンの最中。queue されるので害は無いが、待つのに費用はかからない |
| `waiting` かつ `event === "Notification"` | **待つ** | 許可プロンプトが開いている。**ダイアログはキー入力を読む** |
| `waiting` かつ `event === "Stop"` | 打つ | ターンが終わってプロンプトにいる。**最良の瞬間** |
| 直近 15 秒に人が打鍵 | 待つ | 入力欄に書きかけが残っており、こちらの行と**まとめて submit される** |

最後の 1 つのために `msSinceUserInput`（`write-to-session.ts`）を足した。電話の入力は
Ctrl-C で入力欄を空にしてから貼る（#572）が、**それはユーザがその送信を頼んだからできる**ことで、
**頼まれていないものが人の書きかけを捨ててよい道理は無い**。だから空にはせず、静かになるまで待つ。

**残る穴を正直に**: 15 秒以上放置された書きかけは、こちらの行と一緒に submit される。
入力欄が空かどうかをホストが知る方法は無い。

### W3. 死んだ watch は黙らない

ルールが閉じる・アプリが unpublish される・セッションが reap される。**購読しているつもりで
実は切れている**エージェントは永遠に待つので、それは watch が無いより悪い。だから最後の
リスナーが落ちたところで**終了もまた 1 行として届く**（`onEnded`）。セッションが reap された
ときだけは届けない — 届ける先がもう無いからで、それがこの 2 つの違いである。

### W4. 見るのは「読める範囲」だけ

scope は**実際に 1 件読んで**決める（`readRecords(app, cid, 1)`）。`all` / `own` / `none` の判断と
その理由付けは既に `readRecords` が全部持っているので、ここで作り直せば**2 つの答えが必ずずれる**。
`own` のアプリでは宣言が名指す**身元ごとに 1 本**張る — ルールはどちらの身元も受けるので、
片方だけ見ていたら自分の行の半分を取りこぼす。

**窓は無い**（最初は 200 件だった。やめた）。`limit(n)` の購読は id 順で先頭 n 件しか見ないので、
それより大きいコレクションでは窓の外の変化が**一度も発火しない** — そしてエージェントには
「何も起きていない」と区別がつかない。**沈黙が 2 つのことを意味する watch は watch ではなく、
一番知らせてほしい忙しいアプリでだけ後から発動する罠**である。

費用は正直に: 最初のスナップショットがコレクションの**件数ぶんの read** を 1 度だけ課金する。
その後は**変わった分しか**課金しないので、窓は継続費用を何も抑えていなかった — 初回の read だけを
抑えて、その対価に正しさを払っていた。残る上限は**1 セッション 8 本**のほうである。
read はアプリのプロジェクトに課金されるので、これはツールの prompt でも言う。

### 決めなかったこと

- **セッションをまたがない。** 保存しないし、再起動後に復元しない。復元するというのは、
  他人のアプリが**何時間も後に始まった端末に**キー入力を起こしてよいということで、しかも
  その会話はもう頼んだ覚えが無い。
- **上限は 1 セッション 8 本。** リスナーは張っている間ずっとアプリのオーナーに読みを課金する。
- **バーストは 1 行にまとめる**（1.5 秒）。10 行書かれたのは起きたこと 1 つで、
  10 回起こせば同じ事実を知るのに 10 ターン払う。
