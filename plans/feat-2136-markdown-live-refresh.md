...
room 名に含まれる path は client 由来なので、**他の client 由来パスと同じゲートを通す**。
`resolveContained` は `..` の上昇に加えて **symlink 経由の上昇も拒否する**（自前の字句チェックでは
これが抜ける）。root は「ワークスペース + 生きているセッションの作業ディレクトリ」で、raw file
...
変える。上限が無ければ、接続したページが有効なチャネルに好きなだけ subscribe して切断まで
poll させられる。上限は **socket 単位ではなく watcher 単位**に置く: 消費される資源は poll ループ
であって room ではなく、全体の上限ならページが何枚開いていてもマシン側の stat レートが収まる。
pubsub は file の意味を知らないチャネル輸送のままでいられる。

### 再接続でイベントを落とさない

最後の購読者が消えてから次の購読者が来るまでの間にファイルが変わると、素朴な実装では新しい
watcher が**変更後の状態をベースラインとして採用**してしまい、その変更は誰にも通知されない。
socket が切れた瞬間がまさにこれで、「たまに更新されない」の一番いやな形になる。チャネルごとに
最後に見た stamp を watcher の停止後も保持し、再開時に現在と違えばその場で通知する。

### 何を比べるか

mtime と size だけでは、エージェントの書き込み（temp → rename）を取りこぼし得る。置き換えが
同じ長さで、かつファイルシステムのタイムスタンプ分解能の1刻みに収まると、両方とも変わらない
ことがある。**ファイル識別子（`ino`）を加える** — rename では必ず変わる。Windows の file id は
64bit なので既定の数値型では精度が落ちる。`{ bigint: true }` で読む。
