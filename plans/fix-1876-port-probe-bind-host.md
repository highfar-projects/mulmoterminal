...
**アドレス一致の挙動はプラットフォーム差ではない**（bind できるかどうかは別で、そちらは
プラットフォームによる —— macOS は `127.0.0.2` を bind できず、Linux はできる）。したがって
`127.0.0.53` に bind したサーバへは、**`127.0.0.1` を dial するクライアント**が到達できない。
`127.0.0.53` を dial すれば届く —— 壊れるのは**固定エンドポイントを持つ側**、つまり
`guiMcpUrlTemplate` のように住所をリテラルで書いているクライアントだけ。

（当初ここに「ローカルクライアントから**まったく**到達できない」と書いていた。言い過ぎで、
CodeRabbit が指摘した。壊れる範囲を広く書くと、直す範囲の判断も狂う。）
