...
> **2026-08-18 に半分だけ解消。** ペインは intent を**実行する**ようになった
> （`server/backends/sharedApp/previewIntent.ts`、`plans/feat-shared-app-preview-intent.md`）。
> headless は今も実行しない — 誰も見ていないところで起動されるので、ページを見ただけで
> レコードが動くのは意図ではない。以下はその headless 側の記述として読むこと。

**headless は intent を実行しない。** 会員の書き込み用の経路が無いので、
...
（その非対称は 2026-08-18 に解消した。上の注を見よ。）
