...
| 閉じる導線 3 つ（パネルの閉じるボタン / ツールバーの `+` / Escape） | PASS |
| 拡大（ロスター）時、拡大ターミナルは覆われるが消えない | PASS |
| パネルから shell 起動 → セルが増え、ターミナルが描画される | PASS |
| console エラー / Vue 警告 | なし |

パネルは自分を開いたセルのボタンを覆うので、閉じるのはパネルの閉じるボタン かツールバーの `+` になる。

## パネルは「キーボードだけモーダル」— これが規則

レビューで 3 件（Escape が効かない・フォーカスが戻らない・Escape がショートカットに取られる）が
**別々に**上がってきた。どれも同じ問いへの答えで、1 件ずつ迎え撃つと次も同じ形で来る。規則として
書いておく。

**ポインタは自由、キーボードはパネルのもの。**

| | 挙動 | 理由 |
|---|---|---|
| ポインタ | 背後のターミナルはクリックできる（覆っている範囲を除く） | 見ながら選べることがこの設計の値。背後を `inert` にしない |
| `aria-modal` | **付けない** | 上のとおり非モーダル。付けると支援技術に嘘をつく |
| フォーカストラップ | **作らない** | 同上。Tab で外に出られてよい |
| Escape | どこから押しても閉じる（ターミナル内を除く） | パネルが開いている間、Escape はパネルのもの。ターミナルは例外 — xterm の入力は本物の textarea で、Escape は中のプログラムのもの |
| グリッドショートカット | パネルが開いている間は止まる | 設定モーダルと同じ扱い（`onShortcutKey` の `showSettings` 判定が前例） |
| フォーカス | 開いたら dir 欄、閉じたら開いた要素へ戻る | ただし戻すのはフォーカスがまだパネルのものであるときだけ。起動は意図的に移すし、別所をクリックしていればユーザーが既に選んでいる |

`role="dialog"` は付けるが `aria-modal` は付けない、という組み合わせが非モーダルダイアログの
正しい表現。次に「トラップが無い」と指摘されたら、それは仕様である。
