...
#1828 がバナーの直後に置いたのは、CLAUDE.md のリリース手順の「index は最新リリースの告知バナーで始まる」を「バナーより上には置けない」と読んだため。その一文は「リリースのたびにこのバナーを更新せよ」を言いたいのであって、動画の位置を縛るものではない。最終的にはバナーを先頭に残したまま、動画を「はじめての方へ」の後ろに下げる順にした（下の「変更」）。
