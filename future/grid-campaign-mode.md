...
**落ちて `Orphaned` になったタスクも claim を持ったままにする。** 手放すと、その孤児が
実はまだ途中だった場合に、照合が終わる前に別のタスクが同じパスへ入ってくる。
clone のほうは逆で、`Orphaned` が持っているかどうかは**照合するまで不定**なので、
復帰は `Leased` を通って取り直す。状態ごとの一覧は汎用側 2.1 の表にある。
