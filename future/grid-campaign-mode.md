...
1つのタスクは、**作業をしているあいだ**（`Leased` から `AwaitingApproval` まで）1つの clone を占有し、
...
| | いつ取る | いつ返す | 途中で移れるか |
|---|---|---|---|
| **clone の lease** | `Leased` | `AwaitingApproval` に入るとき | **移れる**。承認待ちから戻るときは `Leased` を通って取り直すので、空いている**別の** clone でよい |
| **パスの claim** | `Claimed`（計画が触る範囲を出してから） | **`Learn` を完了したとき**（停止・エスカレート時は即座に） | 移らない。承認待ちのあいだも、マージ後の書き戻し中も握り続ける |

**lease を取る時点と claim を取る時点がずれているのは意図的である。** 触る範囲は
計画して初めて分かるので、「宣言してから作業場所を取る」順にはできない。
作業場所を取る（`Leased`）→ 読んで計画する → 宣言してパスを claim する（`Claimed`）、が実際の順序で、
下の sequence もその順に並んでいる。
...
**マージした時点でも claim は返さない。** 返すのは書き戻し（`Learn`）が終わったとき。
マージ直後に落ちると、書き戻しが済んでいないのに claim だけ無くなり、
そのパスが誰の仕事だったか分からないまま別のタスクが入ってくる。

**落ちて `Orphaned` になったタスクの claim を、照合が「掃除」してはならない。**
手放すと、その孤児が実はまだ途中だった場合に、照合が終わる前に別のタスクが同じパスへ入ってくる。

ただし**孤児が claim を持っているかどうかはフェーズからは分からない**。
`Planned --> Leased --> Orphaned` は実在の経路で、そのタスクは何も宣言していない。
clone も同じく不定だが、不定の理由が違う — clone は「持っていたか分からない」、
claim は「**どこで落ちたかによる**」。どちらも答えを持っているのは記録のほうで、
照合がそれを読む。復帰は `Leased` を通って clone を取り直す。
状態ごとの一覧は汎用側 2.1 の表にある。
...
**見出しを「開始できない」と書かないのは意図的である。** 決定4で同一 UID を受け入れた以上、
「できない」は言えない — エージェントはキャンペーンの状態ファイルを書けるので、
走っていることにも止まっていることにもできる。マシン上の他のどんなファイルとも同じである。

言えるのは**手段を与えないこと**のほうで、これは設計として守れる。

- キャンペーンを開始・停止する **MCP ツールを与えない**（報告系のみ: 宣言する / 主張する / 結果を返す）
- 開始・停止の route は**エージェントが読めず作れもしない capability** を要求する
- spawn も merge も持たせない

つまり **機能としては開始できない**。**偽装としてはできる**。この差は言葉遊びではなく、
「うっかり起きること」と「そうしようとしたときに起きること」の差で、
守れるのは前者だけだと最初に書いておく。

なお round table の元の不変条件は、ここより**強い**。あちらのエージェントはブラウザの向こうにいて
シェルを持たないので、本当に「できない」と言える。同じ言葉をここへ持ってこないこと。
...
sequenceDiagram
    ...
    G-->>O: 拒否（head が動いた / 最新でない）
    O->>C: clone を取り直して（Leased）再検証・再承認へ
    O->>O: 完了レジストリと決定ログへ書き戻す → 済んだら claim を解放
```

**承認は状態である。** 誰がどの候補 SHA に、**どのベース（取り込んだ main の SHA）に対して**
承認したかを記録し、**どちらかが動いたら無効**にする。別のタスクが先に main を進めれば、
その承認はもう存在しないベースに対するものになる。
無効になったタスクは `Leased` を通って clone を取り直してから、
再取り込み・再検証へ進む（clone を持たないまま `MergeQueue` へは戻れない）。だから承認はキューの先頭で、
...
forge に判定させる**。この保護が有効かは**毎回読んで確かめる** — 人が後から変えられる設定なので、
一度確認したら以後も有効だとは言えない。無効なら鮮度を強制する手段が無いのでマージしない。
拒否されたら **`Leased` を通って clone を取り直してから**キューへ戻して再検証・再承認する
（承認待ちで clone を返しているので、再検証する作業場所がそのままでは無い）。
...
**セッションはステップごとに立てて閉じる。clone は `Leased` から `AwaitingApproval` まで同じものを持つ**（D3）。
...
6. 対象ブランチが指定と一致し、マージに使う資格情報が保護を迂回できない。そして
   **「マージ前にブランチが最新であること」の保護が有効であることを、マージのたびに読んで確かめる**
   （無効なら `expected head SHA` は候補しか守らず、古いベースで検証した候補が入る。
   その場合はマージせずエスカレート）
...
| PR1 | `campaign-state.ts` + store。**intent/outcome レコード、起動時照合、そして claim-registry（期限つき CAS + 所有者トークン）**。UI なし、テストのみ | 再開時の重複実行。**排他が runner より先に存在する** |
| PR2 | **サーバ側の能力境界**（開始/停止/予算の capability、副作用直前の再確認、監査ログ、通知）＋ **ゴール定義の実行権限を deny-by-default で強制**（許可パス・許可コマンド・ネットワーク・資格情報・最大差分）。runner はまだ無い | 「開始する手段を与えない」が方針から強制になる（D4 — 偽装まで防げるとは書かない）。シェルを持つ runner が走り出す前に権限が閉じている |
| PR3 | runner: 1 clone・1タスク・1ステップの walking skeleton | 設計の嘘がここで出る |
| PR4 | turn-watch でステップ連結（1タスクが最後まで自走する） | **現象 D**（勝手に止まる） |
| PR5 | MCP 報告ツール（報告は未検証入力として扱う） | **現象 A**（同じところを直す）— 排他自体は PR1 |
| PR6 | レビュー収束（役割分離・blocking/follow-up・再掲検出・上限） | **現象 B**（20ループ） |
| PR7 | オラクル実行 + 独立検証 | **現象 C**（LGTM 後に出る） |
| PR8 | clone プール 8並列 + マージキュー + レート制限配分。**PR1 の claim-registry が前提** | 規模 |
...
**claim-registry も同じ理由で PR1 に置く。** 当初は MCP 報告ツールと一緒に PR5 に置いていたが、
それだと PR3・PR4 の無人動作が排他の無い状態で走ることになる。`Claimed` の排他と
起動時照合は同じ永続状態の上に載るので、store と一緒に作るのが自然でもある。
**そのうえで PR3・PR4 は並列度1に固定する**（規約ではなく設定として）。
8並列にできるのは PR8 で、それは PR1 の registry に依存する。
...
これは D3 の置き場所側の裏付けでもある。lease を持つのは**セッションではなくタスク**で、
しかも**タスクの一生ぶんでもない**（`Leased` から `AwaitingApproval` までで、承認待ちに入ると返す）。
つまり clone は「誰かのもの」ではなく、runner が貸し出して回収する在庫である。
だから人間の作業ディレクトリの隣ではなく、専用の場所に置く。
