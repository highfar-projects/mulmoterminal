...
**同じ clone・同じ revision で走ることが条件**である。別 clone に渡すと、検証しているのが
どの版か分からなくなる（レビュー済みの版と検証した版が違う、が起きる）。ステップ間で clone は返さない。
