...
| | いつ取る | いつ返す | 途中で移れるか |
|---|---|---|---|
| **clone の lease** | `Claimed` | `AwaitingApproval` に入るとき | **移れる**。承認待ちから戻るときは `Claimed` を通って取り直すので、空いている**別の** clone でよい |
| **パスの claim** | `Claimed` | **`Learn` を完了したとき**（停止・エスカレート時は即座に） | 移らない。承認待ちのあいだも、マージ後の書き戻し中も握り続ける |
...
復帰は `Claimed` を通って取り直す。状態ごとの一覧は汎用側 2.1 の表にある。
...
無効になったタスクは `Claimed` を通って clone を取り直してから、
...
しかも**タスクの一生ぶんでもない**（`Claimed` から `AwaitingApproval` までで、承認待ちに入ると返す）。
