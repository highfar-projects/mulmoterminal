...
1つのタスクは1つの clone を最初から最後まで占有し、その中で Plan / Implement / Review / Verify を
**別々のセッション**として順に立てる。各セッションはそのステップが終わったら**閉じる**。
...
ステップ間で clone は返さない。溜まったセッションは既存の reap に任せる。
...
1. **隔離（唯一の本当の防止）。** エージェントを別 UID / 別 mount namespace / サンドボックスで走らせ、
   キャンペーンの保存先を書けなくする。ただしこれは MulmoTerminal の前提（ローカルの PTY を
   ユーザ自身の権限で動かす）と正面から衝突するので、**既定ではなく明示的な配備オプション**として扱う。
2. **改竄検知 + fail closed（隔離が無いときの上限）。** レコードを**ハッシュ連鎖**にし
   （各レコードが直前のハッシュを持つ）、runner は連鎖の先頭をプロセス内に保持して、
   使う前に毎回照合する。切れていたら**そのキャンペーン全体を止めて人間へ出す**。
   ゴール定義（P1）も起動時に読んで連鎖に固定し、以後はメモリ上の版を権威とする。

**隔離が無い配備での保証は「改竄されない」ではなく「改竄されたらキャンペーンが止まる」である。**
書けてしまうことは消せないので、消せると書かない（停止スイッチの境界と同じ姿勢）。

あわせて、キャンペーンの保存先は**どの clone の外・リポジトリツリーの外**に置く。
そうすればタスクがそこに触ること自体が claim の範囲外になり、自動マージの権限検査で落ちる。
...
| Review / Verify | 自動 | 自動 |
| Merge | **自動**（条件は下記。CI green だけでは足りない） | **人間**（当面） |

`refactor` の自動マージが成立する条件は、**CI が緑であること**ではない。CI が緑という事実は
「CI が何を走らせたか」を言わないので、それ単独では情報量がない。全部満たしたときだけ人間を外す。
...
| PR2 | **サーバ側の能力境界**（開始/停止/予算の capability、副作用直前の再確認、監査ログ、通知）。runner はまだ無い | 「エージェントが開始できない」が方針から強制になる |
...
| PR10 | `refactor` プロファイル（ratchet からの供給、自動マージの権限検査） | 放っておくと進む状態 |
...
- **同一 UID で走る限り、キャンペーンの状態はエージェントから書ける。** 隔離を配備しない場合の
  保証は検知と停止であって防止ではない（4章）。ここを「capability があるから安全」と読み替えない。
