...
- 実行権限は**ガードレールとして実装する**。取り違いと prompt injection による事故は止める。
- **ドキュメントでもコードのコメントでも「安全境界」と呼ばない。** 呼んだ時点で、
  次の読み手はそれに寄りかかった判断をする。
- Campaign Mode の説明（Settings のセクション、skill）に、**無人で並列に走るぶん露出が増える**ことを書く。
  「対象リポジトリの中身は、誰も見ていない状態で8つのエージェントに読まれる」が実態である。
...
PR2 はガードレールとして実装し、安全境界とは呼ばない。露出は Settings と skill の説明に書く。
