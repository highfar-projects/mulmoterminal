...
同一 UID の配備では「安全境界」ではなく「ガードレール」である。** 守るのは取り違いと、
リポジトリの中身に仕込まれた指示（prompt injection）による事故であって、
その気になった相手ではない。ポリシー自体を書き換えられるなら、拒否リストは拒否しない。
...
**どちらを取るかは 9章の未決事項**（決定4）に置く。設計として片方を既定にはしない —
一方は機能を普段の環境で使えなくし、もう一方は露出を受け入れる判断で、どちらも人が決めることである。
...
**決定4（先に要る）: 隔離を必須にするか、通常セルと同じ信頼モデルを受け入れるか。**
4章の表のとおり。同一 UID の配備では実行権限は安全境界ではなくガードレールで、
ポリシー自体を書き換えられる。マージを人間ゲートにしても、コマンド・ネットワーク・資格情報の
濫用はマージを経由しないので減らない。

- **隔離を必須にする**なら、PR2 は「保護されたポリシー保管が無ければ起動を拒否する」を含み、
  PR3（runner）はそれが満たされるまで動かせない。
- **受け入れる**なら、PR2 はガードレールとして実装し、Campaign Mode の説明に
  「無人で走るぶん露出が増える」と書く。

Campaign Mode がそもそも普段の環境で動くかどうかがこれで決まるので、**PR2 の前に決める**。
