...
**どちらも配備しないなら、キャンペーンの権威は信用できない。** これは実装で埋められる差ではないので、
埋めたことにしない。代わりに**結果の側を有界にする**: その信頼領域に不可逆な力を持たせない。

- **自動マージを使わない**（人間ゲート固定）
- **マージに使う資格情報を、キャンペーンの届く範囲に置かない**
- ブランチ保護を、runner の資格情報では迂回できない形にする（自動マージ条件6と同じ話）

つまり隔離なしの配備では、Campaign Mode は「人間が最後に押す並列実行器」であって、
「無人で main を更新する仕組み」ではない。**その線引きを配備の設定として持つ。**
...
- **同一 UID で走る限り、キャンペーンの状態はエージェントから書ける。** ハッシュ連鎖では
  検知にもならない（同じハッシュを計算できる相手だから）。隔離か別権限プロセスを配備しないなら、
  自動マージを使わず、マージの資格情報をキャンペーンの届く範囲に置かない（4章）。
  ここを「capability があるから安全」と読み替えない。
