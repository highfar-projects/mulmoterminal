...
新しい状態を足すときは、この表に行を足せないなら設計がまだ決まっていない。
**逆向きも確かめること** — 実装に在って表に無い状態は、この表を読んだ人には存在しないのと同じである
（`Done` が実際にそうなった。#1837）。

`Done` はこの図の `Learn --> [*]` に名前を与えたもので、実装が足した。終端に名前があると
「`Learn` の完了で claim を解放する」が、`Stopped` / `Escalated` に入るときの解放と同じ形の規則になる。

| 状態 | clone を持つ | claim を持つ | 止められる | 落ちうる |
|---|---|---|---|---|
| `Intake` | — | — | 供給を止める | — |
| `Planned` | — | — | ○ | — |
| `Leased` | ○ | **初回は —、復帰時は ○** | ○ | ○ |
...
| `AwaitingApproval` | — | ○ | ○ | ○ |
| `Merged` | — | ○ | **×** | ○ |
| `Orphaned` | **不定** | **不定** | 解決してから | — |
| `Learn` | — | ○ | **×** | ○ |
| `Done` / `Stopped` / `Escalated` / `Rejected` | — | — | 終端 | 終端 |
...
- **`Leased` は claim について2通りある。** `Planned` から入る初回はまだ持っておらず、
  差し戻し・承認無効・孤児から入る復帰時はすでに持っている。表の「初回は —、復帰時は ○」はその意味で、
  **`Leased` に入ること自体は claim を解放しない**。解放するのは `Learn` の完了と
  `Stopped` / `Escalated` に入った時点だけで、そこは経路によらず同じである。
  復帰時の `Leased --> Claimed` は取り直しではなく素通りになる。
  **ここを「`Leased` は claim を持たない状態」と読むと、復帰のたびに排他が外れる。**
- **`Orphaned` の clone は「不定」で、これは正しい。** 落ちる前に持っていたかどうかは照合するまで
  分からず、持っていても使える保証がない。だから復帰は `Leased` を通って取り直す。
- **`Orphaned` の claim も「不定」で、`clone` とは不定の理由が違う。**
  clone は「持っていたか分からない」、claim は「**どこで落ちたかによる**」である。
  `Planned --> Leased --> Orphaned` は実在の経路で、そのタスクは何も宣言していない。
  フェーズは答えを持っていないので、**照合が記録を読んで決める**。
  ここを「保持している」と書くと、宣言していない孤児を所有者ありとして扱う実装ができる。
- **だが、照合は孤児の claim を「掃除」してはならない。** 不定であることと
  手放してよいことは別である。`Orphaned` は終端ではないので何も解放しない
  （解放は `Learn` の完了と `Stopped` / `Escalated` に入った時点だけ）。
  記録が claim ありと言うなら、それは解決するまで生きている — 手放すと、
  その孤児が実はまだ途中だった場合に、別のタスクが同じパスに入ってくる。
- **`Orphaned` に停止の辺は無い。** 先に解決させる（`Leased` / `Learn` / `Escalated`）。
  停止はその解決先で効く。**マージ済みかどうかを知らないまま止めるのが、いちばん危ない。**
- **`Merged` と `Learn` は止められない。** マージはすでに外で起きた事実で、止める対象が無い。
  停止がマージ後に届いたタスクは止まらず、記録まで進んで終わる。
  「停止したのに書き戻しが走る」のは正しい振る舞いで、漏れではない。
- **claim を解放するのは `Learn` を完了したときと、`Stopped` / `Escalated` に入った時点。**
  `Merged` に入った時点では解放しない — マージ直後に落ちたとき、
  そのパスが誰の仕事だったかを残しておくため。
...
- **承認は「候補 SHA」と「検証したベース」の組に対して与える。** ベースとは、
  ...
  **どちらかが動いたら承認は無効**になる。`Leased` を通って clone を取り直してから
  取り込み直し、再検証し、もう一度聞く（承認待ちのタスクは clone を持っていないので、
  `MergeQueue` へ直接は戻れない）。パスの claim は持ったままなので、取り直すのは作業場所だけ。
...
- **差し戻しは `Implementing` へ直行しない。** 承認待ちのタスクは clone を持っていないので、
  まず `Leased` に戻って**空いている clone を取り直す**（パスの claim は握ったままなので、
...
| Leased / Claimed | 作業場所を借りて計画し、触るパスと提供/依存する契約を宣言して排他を取る | 同じところを2人が同時に直す |
...
7. **対象ブランチが P1 の指定と一致**し、そのブランチの必須保護が**実際に有効**で、
   マージに使う資格情報がそれを**迂回できない**。とくに
   **「マージ前にブランチが最新であること」（GitHub の branch protection の `strict`）が
   有効であることを、マージのたびに読んで確かめる。**
   これが無効だと、`expected head SHA` は候補の head しか守らないので、
   **古いベースで検証した候補がそのまま入る**。
   無効だったら**マージしない**（人間ゲートでも runner は実行しない）。エスカレートする。
   自前の事前チェックで代用しないこと — それは競合する（2.1.1）
