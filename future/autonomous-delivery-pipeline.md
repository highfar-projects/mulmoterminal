...
| Gate | 既定はマージ前のみ人間。慣れたら CI green で自動マージ |
...
### 4.4 なぜ worktree ではなく clone を8個か
...
| ID | 機能 | 解く問題 |
|---|---|---|
| P1 | **ゴール定義ファイル**（対象リポ、プロファイル、完了条件、並列数、予算） | 全体の入力 |
...
| P7 | **監査ログ**。どのセッションがどの clone で何をしたか | 事故調査 |
...
| ID | 機能 | 解く問題 |
|---|---|---|
| V1 | **Done 条件の機械判定**（テストが増えた / pure 関数化された / `any` が増えていない / 公開 API 差分なし） | 判定を人間の記憶に置かない |
...
| W3 | **main 同期ポリシー**（着手前と各コミット前に取り込む。merge、rebase しない） | 現象 A / K3 |
