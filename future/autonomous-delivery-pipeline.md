...
- **パスの claim を持つ状態**は `Claimed` から `Merged` までの全部と `AwaitingApproval`、そして `Learn`。
  解放するのは、`Stopped` / `Escalated` に入った**とき**（もう何も起きないので即座に）と、
  `Learn` を**完了したとき**（マージ済みの書き戻しの途中で落ちた場合に、
  そのパスが誰の仕事だったかを残しておくため。`Merged` に入った時点では解放しない）。
- **止められる状態からは `Stopped` の辺が要る。止められる状態とは `Planned` から
  `AwaitingApproval` まで**である。`Merged` と `Learn` にはこの辺が**無い** —
  マージはすでに外で起きた事実で、止める対象が無いからである（下記）。
  停止がマージ後に届いたタスクは止まらず、記録まで進んで終わる。
  「停止したのに書き戻しが走る」のは正しい振る舞いで、漏れではない。
- **落ちうる状態からは `Orphaned` の辺が要る。落ちうる状態は `Claimed` 以降のすべて**で、
  こちらには `Merged` と `Learn` も含まれる。lease を取った時点から失うものがあり、
  書き戻しの途中でもプロセスは落ちる。`Planned` にこの辺が無いのは、
  まだ何も取っておらず、再起動してもそのまま `Planned` だからである。

**辺を1本足したら、その状態が「止められるのか」「落ちうるのか」を別々に答える。**
この2つを1つの規則にまとめようとしたのが、ここに最初に書いた不変条件の誤りだった。
