...
5. **強制はエージェントの外側で行う。ただし「外側」はプロセス境界であってファイル権限でもある。**
   ...
   API に capability を付けても、状態ファイルは直接書き換えられる。隔離できないなら
   ハッシュ連鎖で改竄を検知して fail closed にする — 保証は「改竄されない」ではなく
   「改竄されたら止まる」であり、そう書く。
...
| Gate | 既定はマージ前のみ人間。慣れたら自動マージ（条件は 4.4） |
...
CI が緑であることは、**CI が何を走らせたか**を言わない限り情報量がない。
このプロファイルで人間を外すときは、次を全部満たしたときだけとする。満たせないものが1つでもあれば人間ゲートへ倒す。
